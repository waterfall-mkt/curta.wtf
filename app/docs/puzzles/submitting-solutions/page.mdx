...
Flag NFTs are minted to a solver upon a valid submission and are the base credential for solving a puzzle. 100% of solve data is stored onchain, but Flag NFTs package the data in a prettier, more human-readable format:
