...
const CTFFeature: FC = () => {
  ...
  // Opacity animation `keytimes` of each Flag skeleton element.
  const FLAG_KEYTIMES = '0;0.145;0.16;0.6;0.625;1';
  // Width of the flag.
  const flagWidth = (HEIGHT * 264) / 412;
  // Starting `x` coordinate of the flag.
  const flagX = WIDTH - flagWidth;
  // Length of the dashed lines leading into the icon circle.
  const strokeLength = (WIDTH - CIRCLE_SIZE) / 2 - flagWidth;
  // Length of each dash in the dashed lines leading into the icon circle. Note
  // that we want the following shape:
  //                              -__--__--__--__-,
  // where `-` is a dash and `_` is a space. This way, a dash leading from/into
  // an object is always half a dash length.
  const strokeDash = strokeLength / 8;
  const strokeDasharray = `${strokeDash / 2},${(strokeDash.toString() + ',').repeat(7)}${
    strokeDash / 2
  }`;
  // Flag border radius w/ margin for 1px stroke width.
  const flagBorderRadius = 4 * (HEIGHT / 406) - 0.5;
  const circleRadius = CIRCLE_SIZE / 2 - 0.5;
  // Path the gradient runs along.
  const path =
    // Starting point; Leave room for 1px stroke width.
    `M${WIDTH - 0.5} ${HEIGHT / 2}` +
    // Move up to top; leave room for turn and 1px stroke width.
    `v-${HEIGHT / 2 - flagBorderRadius - 0.5}` +
    // Quarter turn to top and left.
    `a${flagBorderRadius} ${flagBorderRadius} 0 0 0 -${flagBorderRadius} -${flagBorderRadius}` +
    // Move left to left edge of Flag; leave room for turn and 1px stroke width.
    `h-${flagWidth - 2 * flagBorderRadius - 1}` +
    // Quarter turn to bottom and left.
    `a${flagBorderRadius} ${flagBorderRadius} 0 0 0 -${flagBorderRadius} ${flagBorderRadius}` +
    // Move down to vertical center of Flag; leave room for 1px stroke width.
    `v${HEIGHT / 2 - flagBorderRadius - 0.5}` +
    // Move left to right edge of icon circle; leave room for 1px stroke width.
    `h-${strokeLength + 1}` +
    // Quarter turn to top and left.
    `a${circleRadius} ${circleRadius} 0 0 0 -${circleRadius} -${circleRadius}` +
    // Quarter turn to bottom and left.
    `a${circleRadius} ${circleRadius} 0 0 0 -${circleRadius} ${circleRadius}` +
    // Move left to edge of the code block; leave room for 1px stroke width.
    `h-${strokeLength + 0.5}` +
    // Move right to the edge of the icon circle; leave room for 1px stroke
    // width.
    `h${strokeLength + 0.5}` +
    // Quarter turn to bottom and right.
    `a${circleRadius} ${circleRadius} 0 0 0 ${circleRadius} ${circleRadius}` +
    // Quarter turn to top and right.
    `a${circleRadius} ${circleRadius} 0 0 0 ${circleRadius} -${circleRadius}` +
    // Move right to left edge of the Flag; leave room for 1px stroke width.
    `h${strokeLength + 1}` +
    // Move down to bottom of Flag; leave room for turn and 1px stroke width.
    `v${HEIGHT / 2 - flagBorderRadius - 0.5}` +
    // Quarter turn to bottom and right.
    `a${flagBorderRadius} ${flagBorderRadius} 0 0 0 ${flagBorderRadius} ${flagBorderRadius}` +
    // Move right to right edge of Flag; leave room for turn and 1px stroke.
    `h${flagWidth - 2 * flagBorderRadius - 1}` +
    // Quarter turn to top and right.
    `a${flagBorderRadius} ${flagBorderRadius} 0 0 0 ${flagBorderRadius} -${flagBorderRadius}` +
    // Move up to vertical center of Flag.
    `v${HEIGHT / 2 - flagBorderRadius - 0.5}` +
    'z';
  ...
  return (
    <svg
      ...
      role="img"
    >
      <title>Curta Puzzles</title>
      <desc>Animated graphic of Curta Puzzles.</desc>
      <defs>
        {/* Pulse gradient from the end of the code block to around the icon
        and the flag. */}
        <motion.linearGradient
          animate={{ x1: [0, WIDTH * 2], x2: [0, WIDTH] }}
          transition={{ delay: 0.7, duration: 2, repeat: Infinity, repeatDelay: 8 }}
          id="pulse"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="rgba(117, 129, 149, 1)" stopOpacity="0" />
      ...
      <rect
        ...
        strokeWidth="1"
      />
      {/* Dashed line from code block to icon circle. */}
      <line
        className="stroke-stroke"
        x1={flagWidth}
        y1={HEIGHT / 2 - 0.5}
      ...
      />
      {/* Dashed line from icon circle to flag. */}
      <line
        className="stroke-stroke"
        x1={flagWidth + strokeLength + CIRCLE_SIZE}
        y1={HEIGHT / 2 - 0.5}
        x2={flagWidth + 2 * strokeLength + CIRCLE_SIZE}
        y2={HEIGHT / 2 - 0.5}
        strokeDasharray={strokeDasharray}
      />
      {/* Icon circle border. */}
      ...
      <circle className="fill-gray-600" r={CIRCLE_SIZE / 2 - 1} cx={WIDTH / 2} cy={HEIGHT / 2} />
      {/* Icon. */}
      <g
        transform={`translate(${WIDTH / 2 - CIRCLE_SIZE / 4} ${
          HEIGHT / 2 - CIRCLE_SIZE / 4
        }) scale(${CIRCLE_SIZE / 48})`}
      >
      ...
      <g transform={`translate(${flagX} 0)`}>
        ...
        >
          ...
          <defs>
            {/* Mask to round corners of the Drunken Bishop viz heatmap. */}
          ...
          {/* Phase stars skeleton. */}
          <rect className="fill-gray-250" x="176" y="28" width={64} height={24} rx={12} />
          {/* We group here to collectively animate the Phase stars in and out
          as a group. */}
          <g>
          ...
          {/* Solution Drunken Bishop viz heatmap. */}
          <g
            xmlns="http://www.w3.org/2000/svg"
          ...
          {/* Skeleton primary group. */}
          <g className="fill-gray-250">
            {/* Puzzle header skeleton. */}
            <rect width="84" height="22" x="20" y="20" rx="4" />
            {/* ``Captured by`` skeleton. */}
            <rect width="72" height="20" x="44" y="306" rx="4" />
            {/* ``Solution`` skeleton. */}
            <rect width="72" height="20" x="160" y="306" rx="4" />
            {/* ``Rank`` skeleton. */}
            <rect width="72" height="20" x="44" y="356" rx="4" />
            {/* ``Solve time`` skeleton. */}
            <rect width="72" height="20" x="160" y="356" rx="4" />
            <animate
              attributeName="opacity"
              values="1;1;0;0;1;1"
              keyTimes={FLAG_KEYTIMES}
          ...
          {/* Skeleton secondary group. */}
          <g className="fill-gray-350">
            {/* ``Created by`` skeleton. */}
            <rect width="120" height="14" x="20" y="46" rx="4" />
            {/* ``Captured by`` skeleton. */}
            <rect width="20" height="20" x="20" y="306" rx="4" />
            <rect width="72" height="14" x="44" y="328" rx="4" />
            {/* ``Solution`` skeleton. */}
            <rect width="20" height="20" x="136" y="306" rx="4" />
            <rect width="72" height="14" x="160" y="328" rx="4" />
            {/* ``Rank`` skeleton. */}
            <rect width="20" height="20" x="20" y="356" rx="4" />
            <rect width="72" height="14" x="44" y="378" rx="4" />
            {/* ``Solve time`` skeleton. */}
            <rect width="20" height="20" x="136" y="356" rx="4" />
            <rect width="72" height="14" x="160" y="378" rx="4" />
            <animate
              attributeName="opacity"
              values="1;1;0;0;1;1"
              keyTimes={FLAG_KEYTIMES}
      ...
      {/* Pulse gradient path from the end of the code block to around the icon
      and the flag. */}
      <path d={path} stroke="url(#pulse)" strokeLinecap="round" strokeWidth="1" />
    </svg>
  );
};
