...
export const golfCourseCommits = pgTable('golf_course_commits', {
  chainId: integer('chain_id').notNull(),
...
export const golfCourseSolves = pgTable('golf_course_solves', {
  // On-chain identifier
  ...
  pk: primaryKey({ columns: [table.courseId, table.chainId, table.solverAddress, table.submitTx] }),
  courseIdChainIdIdx: index('golf_course_solves_course_id_chain_id_idx').on(table.courseId, table.chainId),
...
export const puzzleSolves = pgTable('puzzle_solves', {
  ...
  puzzleIdChainIdIdx: index('puzzle_solves_puzzle_id_chain_id_idx').on(table.puzzleId, table.chainId),
...
export const teamMemberApprovals = pgTable('team_member_approvals', {
  ...
  teamIdChainIdIdx: index('team_member_approvals_team_id_chain_id_idx').on(table.teamId, table.chainId),
  userAddressIdx: index('team_member_approvals_user_address_idx').on(table.userAddress),
}));

export const teamTransfers = pgTable('team_transfers', {
  id: varchar('id', { length: 255 }).primaryKey().default(generateCuid()),
  userAddress: varchar('user_address', { length: 255 }).notNull(),
  chainId: integer('chain_id').notNull(),
  ...
  fromTeamIdChainIdIdx: index('team_transfers_from_team_id_chain_id_idx').on(table.fromTeamId, table.chainId),
  toTeamIdChainIdIdx: index('team_transfers_to_team_id_chain_id_idx').on(table.toTeamId, table.chainId),
}));
...
export const accounts = pgTable('accounts', {
  ...
  providerProviderAccountIdUnique: uniqueIndex('accounts_provider_provider_account_id_unique').on(table.provider, table.providerAccountId),
}));
...
export const verificationTokens = pgTable('verification_tokens', {
  ...
  identifierTokenUnique: uniqueIndex('verification_tokens_identifier_token_unique').on(table.identifier, table.token),
}));
